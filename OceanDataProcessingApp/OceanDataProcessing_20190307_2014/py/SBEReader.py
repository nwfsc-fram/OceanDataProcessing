...
class SbeReader:
    ...
    def _get_model(self):
        ...
        logging.info(f"self._latitude = {self._latitude}")
        logging.info(f"self._longitude = {self._longitude}")
        logging.info(f"self._start_date_time = {self._start_date_time}")

        if self._latitude is None or np.isnan(self._latitude):
            logging.info(f"\t\tLatitude does not exist, skipping parsing this cast")
            return status
        if self._longitude is None or np.isnan(self._longitude):
            logging.info(f"\t\tLongitude does not exist, skipping parsing this cast")
            return status
        if self._start_date_time is None:
            logging.info(f"\t\tStart date/time does not exist, skipping parsing this cast")
            return status
        ...
        logging.info(f"model retrieval is completed")

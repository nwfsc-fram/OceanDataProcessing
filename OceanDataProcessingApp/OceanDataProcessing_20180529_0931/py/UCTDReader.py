...
class UctdReader:
    ...
    def set_support_df(self, filename=None):
        """
        ...
        for i, line in enumerate(self._raw_data.splitlines()):
            ...
            if "*Lat" in line:
                ...
                if len(latitude) > 2:
                    ...
                    latitude = None
                    logging.info(f"\t\tMissing Latitude in source file")
            ...
            if "*Lon" in line:
                ...
                if len(longitude) > 3:
                    ...
                    longitude = None
                    logging.info(f"\t\tMissing Longitude in source file")
                instrument["Longitude"] = longitude

            if "*Cast" in line and len(line.split(" ")) >= 6 and "start" not in line:
                dt = line.split(" ")
                # logging.info(f"dt = {dt}")
                dt = arrow.get(f"{dt[2]} {dt[3]} {dt[4]} {dt[5]}", "DD MMM YYYY HH:mm:ss", tzinfo=tz.gettz('US/Pacific'))
                instrument["DateTime"] = dt.isoformat()
    ...
    def parse_data(self, output="memory"):
        ...
        if self._latitude:
            ...
            source = 'locs file'
        elif "Latitude" in self._instrument and self._instrument["Latitude"]:
            latitude = self._instrument["Latitude"]
            source = 'instrument'
        logging.info(f"chosen lat = {latitude} >>> from {source}")
        ...
        if self._longitude:
            ...
            source = 'locs file'
        elif "Longitude" in self._instrument and self._instrument["Longitude"]:
            longitude = self._instrument["Longitude"]
            source = 'instrument'
        logging.info(f"chosen lon = {longitude} >>> from {source}")
